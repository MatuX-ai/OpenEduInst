...
export class OrgAdminService {
  ...
  getOrgOverview(orgId: number): Observable<OrgOverview> {
    ...
    return this.http
      .get<
        ApiResponse<OrgOverview>
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/overview`, { headers })
      .pipe(
        timeout(5000), // 5秒超时
        retry(2), // 失败重试2次
        map((response) => {
  ...
  getOrgCourses(orgId: number): Observable<CourseInfo[]> {
    ...
    return this.http
      .get<
        ApiResponse<PaginatedResponse<CourseInfo>>
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/courses`, { headers })
      .pipe(
        timeout(5000),
        retry(2),
        map((response) => {
  ...
  getEnrollmentStats(orgId: number): Observable<EnrollmentStats> {
    ...
    return this.http
      .get<
        ApiResponse<EnrollmentStats>
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/enrollment/stats`, { headers })
      .pipe(
        timeout(5000),
        retry(2),
        map((response) => {
  ...
  getOrgTeachers(orgId: number): Observable<TeacherInfo[]> {
    ...
    return this.http
      .get<
        ApiResponse<PaginatedResponse<TeacherInfo>>
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/teachers`, { headers })
      .pipe(
        timeout(5000),
        retry(2),
        map((response) => {
  ...
  getOrgStudents(orgId: number): Observable<StudentInfo[]> {
    ...
    return this.http
      .get<
        ApiResponse<PaginatedResponse<StudentInfo>>
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/students`, { headers })
      .pipe(
        timeout(5000),
        retry(2),
        map((response) => {
  ...
  getCourseStats(orgId: number): Observable<CourseStats> {
    ...
    return this.http
      .get<
        ApiResponse<CourseStats>
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/course/stats`, { headers })
      .pipe(
        timeout(5000),
        retry(2),
        map((response) => {
  ...
  getOrgDashboard(orgId: number): Observable<OrgDashboardData> {
    ...
    return this.http
      .get<
        ApiResponse<OrgDashboardData>
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/dashboard`, { headers })
      .pipe(
        timeout(10000), // 10秒超时
        retry(2),
        map((response) => {
  ...
  createCourse(orgId: number, courseData: Partial<CourseInfo>): Observable<CourseInfo> {
    ...
    return this.http
      ...
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/courses`, courseData, { headers })
      .pipe(
        timeout(8000),
        map((response) => {
          if (response.success && response.data) {
  ...
  updateCourse(
    ...
    return this.http
      .put<
        ApiResponse<CourseInfo>
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/courses/${courseId}`, courseData, { headers })
      .pipe(
        timeout(8000),
        map((response) => {
          if (response.success && response.data) {
  ...
  addTeacher(orgId: number, teacherData: Partial<TeacherInfo>): Observable<TeacherInfo> {
    ...
    return this.http
      .post<
        ApiResponse<TeacherInfo>
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/teachers`, teacherData, { headers })
      .pipe(
        timeout(8000),
        map((response) => {
          if (response.success && response.data) {
  ...
  addStudent(orgId: number, studentData: Partial<StudentInfo>): Observable<StudentInfo> {
    ...
    return this.http
      ...
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/students`, studentData, { headers })
      .pipe(
        timeout(8000),
        map((response) => {
          if (response.success && response.data) {
  ...
  updateStudentProgress(
    ...
    return this.http
      .put<
        ApiResponse<StudentInfo>
      >(`${this.EDUCATION_API_BASE}/org/${orgId}/students/${studentId}/progress`, progressData, { headers })
      .pipe(
        timeout(8000),
        map((response) => {
          if (response.success && response.data) {
   ...
   * 获取多源学习整合的教师数据
   */
  getMultiSourceTeacherData(teacherId: number): Observable<{
    teacher_id: number;
  ...
  private getSimulatedOrgOverview(_orgId: number): Observable<OrgOverview> {
    return of({
  ...
  private getSimulatedCourses(orgId: number): Observable<CourseInfo[]> {
    ...
    return of(
      Array.from({ length: 5 }, (_, i) => ({
        ...
        enrollmentCount: Math.floor(Math.random() * 30 + 10),
        capacity: 20,
  ...
  private getSimulatedEnrollmentStats(_orgId: number): Observable<EnrollmentStats> {
    return of({
  ...
  private getSimulatedTeachers(orgId: number): Observable<TeacherInfo[]> {
    ...
    const departments = ['编程教研组', '机器人教研组', '创客教研组', 'AI教研组', '硬件教研组'];
    return of(
      Array.from({ length: 5 }, (_, i) => ({
        id: i + 1,
        user_id: 1000 + i,
  ...
  private getSimulatedCourseStats(_orgId: number): Observable<CourseStats> {
    return of({
      totalCourses: 15,
      activeCourses: 8,
      completedCourses: 5,
  ...
  /**
   * 获取机构 Dashboard 核心指标（MatuX 风格）
   */
  getOrgMetrics(orgId: number): Observable<{ activeStudents: number; monthlyRevenue: string; courseCompletionRate: string }> {
    const headers = this.getAuthHeaders();
    return this.http.get<ApiResponse<any>>(`${this.EDUCATION_API_BASE}/org/${orgId}/metrics`, { headers }).pipe(
      map((response) => response.data || { activeStudents: 0, monthlyRevenue: '¥0', courseCompletionRate: '0%' }),
      catchError(() => of({ activeStudents: 328, monthlyRevenue: '¥12.5W', courseCompletionRate: '92%' }))
    );
  }
  ...
  deleteCourse(courseId: number): Observable<{ success: boolean; message?: string }> {
    ...
    return this.http
      ...
      >(`${this.EDUCATION_API_BASE}/courses/${courseId}`, { headers })
      .pipe(
        timeout(5000),
        map((response) => {
          if (response.success && response.data) {
