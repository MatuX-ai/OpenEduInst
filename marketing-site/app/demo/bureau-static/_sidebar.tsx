  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200 flex-shrink-0 flex flex-col">
      <div className="p-5 border-b border-slate-200">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="font-bold text-slate-800 text-sm truncate">梅山县教育局</h1>
            <p className="text-xs text-slate-500 truncate">STEM 监管平台</p>
          </div>
        </div>
      </div>
            ...
            {item.badge && (
              <span
                className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${
                  item.badge.includes("待处理") || item.badge.includes("46")
      ...
      <div className="p-4 border-t border-slate-200">
        <Link
          href="/demo"
          className="flex items-center justify-center gap-2 px-4 py-2.5 text-sm text-slate-600 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-all"
        >
