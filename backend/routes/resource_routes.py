...
def get_resource_stats(
    ...
    """获取资源统计数据"""
    # 总资源数
    total_resources = db.query(TeachingResource).filter(
        TeachingResource.org_id == org_id
    ).count()
    
    # 本月下载量
    first_day_of_month = datetime.now().replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    monthly_downloads = db.query(TeachingResource).filter(
        TeachingResource.org_id == org_id,
        TeachingResource.last_download_time >= first_day_of_month
    ).with_entities(
        db.func.sum(TeachingResource.download_count)
    ).scalar() or 0
    
    # 视频总时长（估算，假设每个视频平均30分钟）
    video_count = db.query(TeachingResource).filter(
        TeachingResource.org_id == org_id,
        TeachingResource.resource_type == ResourceType.VIDEO
    ).count()
    video_hours = video_count * 0.5  # 假设平均每个视频30分钟
    
    # 代码示例数量
    code_count = db.query(TeachingResource).filter(
        TeachingResource.org_id == org_id,
        TeachingResource.resource_type == ResourceType.CODE
    ).count()
    ...
    categories = db.query(
        ...
        TeachingResource.org_id == org_id
    ).group_by(TeachingResource.category).all()
...
def get_categories(
    ...
    categories = db.query(ResourceCategory).filter(
        ResourceCategory.org_id == org_id
    ).order_by(ResourceCategory.sort_order.asc()).all()
    
    # 更新每个分类的资源数量
    for category in categories:
        count = db.query(TeachingResource).filter(
            TeachingResource.org_id == org_id,
...
def record_download(
    ...
    """记录资源下载"""
    resource = db.query(TeachingResource).filter(
