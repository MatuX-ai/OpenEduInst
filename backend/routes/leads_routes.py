...
from models.schedule import Lead, LeadFollowUp, LeadStatus
...
def get_lead_stats(
    ...
    monthly_leads = db.query(Lead).filter(
        Lead.org_id == org_id,
        Lead.create_time >= first_day_of_month
    ).count()
    ...
    enrolled_leads = db.query(Lead).filter(
        Lead.org_id == org_id,
...
def get_follow_ups(
    ...
    ).order_by(LeadFollowUp.follow_up_time.desc()).all()
