...
def update_social_account(
    ...
    account = (
        ...
        .filter(SocialMediaAccount.id == account_id, SocialMediaAccount.org_id == org_id)
        .first()
    )
    if not account:
...
def get_campaign_stats(
    ...
    monthly_revenue = (
        ...
        .with_entities(db.func.sum(MarketingCampaign.revenue))
        .scalar() or 0
    )
    ...
    active_participants = (
        ...
        .with_entities(db.func.sum(MarketingCampaign.participants_count))
        .scalar() or 0
    )
    ...
    total_coupons = (
        db.query(Coupon)
        .filter(Coupon.org_id == org_id)
        .with_entities(db.func.sum(Coupon.total_quantity))
        .scalar() or 0
    )
    ...
    used_coupons = (
        db.query(Coupon)
        .filter(Coupon.org_id == org_id)
        .with_entities(db.func.sum(Coupon.used_quantity))
        .scalar() or 0
    )
...
def update_campaign_status(
    ...
    campaign = (
        db.query(MarketingCampaign)
        .filter(MarketingCampaign.id == campaign_id, MarketingCampaign.org_id == org_id)
        .first()
    )
...
def create_coupon(
    ...
    return {"message": "Coupon created successfully", "coupon": new_coupon.to_dict()}
...
def use_coupon(
    ...
    coupon = (
        db.query(Coupon)
        .filter(Coupon.id == coupon_id, Coupon.org_id == org_id)
        .first()
    )
