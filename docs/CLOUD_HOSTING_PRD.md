...
#### 2.2.5 OpenMTSciEd STEM 资源集成

**功能概述**: 云托管版通过 EduInst 后端代理接入 OpenMTSciEd 平台，为机构管理员与授课教师提供教程、课件、硬件项目等 STEM 数字资源，与机构物理设备台账（`/devices`）职责分离。

**✅ 当前实现状态**:
- 组织模型已预留 `opensciedu_*` 字段（`backend/models/license.py`）
- 环境变量已配置 `OPENSCIEDU_API_BASE` / `OPENSCIEDU_API_KEY` / `OPENSCIEDU_CDN_BASE`（`backend/config/settings.py`）
- 数据库迁移脚本已提供（`backend/scripts/migrate_opensciedu.py`）
- **M1 已交付**: 后端代理 `/api/v1/opensciedu/*`、前端 `OpenMtSciEdService`、单元测试与联调脚本
- **M3 已交付**: 教师工作台、角色守卫、菜单过滤、系统设置集成 Tab
- **已实现 (M6)**: `@openmt/scied-ui` 共享 Angular 组件库（Monorepo + ng-packagr）
- **未实现**: 课题工作室 SSO 免登

**🎯 规划目标（详见 [OPENMTSCIED_INTEGRATION_PRD.md](./OPENMTSCIED_INTEGRATION_PRD.md)）**:

| 需求项 | 规格说明 |
|-------|---------|
| **代理 API** | `/api/v1/opensciedu/tutorials` · `coursewares` · `hardware-projects` · `stats` · `health` |
| **教学资源页** | 替换 Mock，Tab 浏览 SciEd 教程/课件/硬件项目 |
| **教师工作台** | `/organization/:id/teacher/*`，角色守卫 |
| **机构配置** | 启用开关、API Key、测试连接 |
| **仪表盘** | 资源卡片跳转 + 真实数量统计 |

> 实施计划：[plans/openmtscied-integration-plan.md](./plans/openmtscied-integration-plan.md)
...
- [OpenMTSciEd 集成 PRD](./OPENMTSCIED_INTEGRATION_PRD.md)
- [OpenMTSciEd 集成实施计划](./plans/openmtscied-integration-plan.md)
