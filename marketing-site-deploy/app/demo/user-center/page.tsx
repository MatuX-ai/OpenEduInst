...
const ADMIN_BASE = process.env.NEXT_PUBLIC_ADMIN_URL || "http://localhost:4200";
  ...
  const redirectToOrganization = useCallback(async () => {
    try {
      ...
      if (organizations.length === 0) {
        redirectUrl = `${ADMIN_BASE}/organization/1/dashboard`;
      } else {
        const org = organizations[0];
        const token = localStorage.getItem("access_token");
        redirectUrl = `${ADMIN_BASE}/organization/${org.id}/dashboard?token=${encodeURIComponent(token || "")}`;
      }
      ...
      await new Promise((resolve) => setTimeout(resolve, 600));
      const token = localStorage.getItem("access_token");
      window.location.href = `${ADMIN_BASE}/organization/1/dashboard?token=${encodeURIComponent(token || "")}`;
    }
  }, []);
